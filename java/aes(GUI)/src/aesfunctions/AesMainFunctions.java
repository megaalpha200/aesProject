...
public class AesMainFunctions {
    ...
    public static void main(String[] args) {
        //debugOutput.append(aesDecrypt("ff0b844a0853bf7c6934ab4364148fb9", "0f1571c947d9e8590cb7add6af7f6798"));
    }

    public static String aesEncrypt(String plaintextHex, String initialKeyHex) {
        final int maxPlainTextHexSize = (int) Math.ceil(plaintextHex.length() / 32.0) * 32;
        final String paddedPlaintextHex = StringManipulations.padRightWithZeros(plaintextHex, maxPlainTextHexSize);
        final List<String> plaintextBlocks = StringManipulations.chunkString(paddedPlaintextHex, 32);
        final String paddedInitialKey = StringManipulations.padRightWithZeros(initialKeyHex, 32).substring(0, 32);
        ...
        for (String block : plaintextBlocks) {
            ...
            for (int round = 1; round <= 10; round++) {
                ...
                debugOutput.append("Round ").append(round).append(" Key: ").append(roundKeys.get(round)).append(System.lineSeparator());
                ...
                roundStates.add(roundFunction(CipherMode.ENCRYPT, tempRoundObject, roundKeys.get(round), round != 10));
            }

            debugOutput.append(System.lineSeparator()).append(System.lineSeparator());

            debugOutput.append("For Block ").append(blockNum + 1).append("...").append(System.lineSeparator());
            debugOutput.append("Block State: ").append(String.join(" ", StringManipulations.chunkString(block, 2))).append(System.lineSeparator());
    ...
    public static String aesDecrypt(String ciphertextHex, String initialKeyHex) {
        final int maxCipherTextHexSize = (int) Math.ceil(ciphertextHex.length() / 32.0) * 32;
        final String paddedCiphertextHex = StringManipulations.padRightWithZeros(ciphertextHex, maxCipherTextHexSize);
        final List<String> ciphertextBlocks = StringManipulations.chunkString(paddedCiphertextHex, 32);
        final String paddedInitialKey = StringManipulations.padRightWithZeros(initialKeyHex, 32).substring(0, 32);
        final ArrayList<String> finalCipherTextList = new ArrayList<>();
        ...
        for (String block : ciphertextBlocks) {
            ...
            for (int round = 1; round <= 10; round++) {
                ...
                roundStates.add(roundFunction(CipherMode.DECRYPT, tempRoundObject, roundKeys.get(round), round != 10));
            }
    ...
    private static void displayRoundStates(List<RoundObject> roundStates, int blockNum, CipherMode cipherMode) {
        int roundNum = 0;
        for (RoundObject roundObj : roundStates) {
            debugOutput.append("For Block " + (blockNum+1) + ", Round " + roundNum + "...").append(System.lineSeparator());

            if (roundObj.isPreRound())
                debugOutput.append("After Pre-Round: " + String.join(" ", StringManipulations.chunkString(roundObj.getPreRoundState(), 2))).append(System.lineSeparator());
            else {
                if (cipherMode == CipherMode.ENCRYPT) {
                    debugOutput.append("After SubBytes: " + String.join(" ", StringManipulations.chunkString(roundObj.getSubBytesState(), 2))).append(System.lineSeparator());
                    debugOutput.append("After ShiftRows: " + String.join(" ", StringManipulations.chunkString(roundObj.getShiftRowsState(), 2))).append(System.lineSeparator());
                    debugOutput.append("After MixColumns: " + String.join(" ", StringManipulations.chunkString(roundObj.getMixColumnsState(), 2))).append(System.lineSeparator());
                    debugOutput.append("After AddRoundKey: " + String.join(" ", StringManipulations.chunkString(roundObj.getAddRoundKeyState(), 2))).append(System.lineSeparator());
                }
                else if (cipherMode == CipherMode.DECRYPT) {
                    debugOutput.append("After InvShiftRows: " + String.join(" ", StringManipulations.chunkString(roundObj.getShiftRowsState(), 2))).append(System.lineSeparator());
                    debugOutput.append("After InvSubBytes: " + String.join(" ", StringManipulations.chunkString(roundObj.getSubBytesState(), 2))).append(System.lineSeparator());
                    debugOutput.append("After AddRoundKey: " + String.join(" ", StringManipulations.chunkString(roundObj.getAddRoundKeyState(), 2))).append(System.lineSeparator());
                    debugOutput.append("After InvMixColumns: " + String.join(" ", StringManipulations.chunkString(roundObj.getMixColumnsState(), 2))).append(System.lineSeparator());
                }
            }
    ...
    private static RoundObject roundFunction(CipherMode cipherMode, RoundObject roundObj, String roundKey, boolean useMixCols) {
